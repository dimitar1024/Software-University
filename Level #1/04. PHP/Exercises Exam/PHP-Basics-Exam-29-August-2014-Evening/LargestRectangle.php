...
list($minRow, $minCol, $maxRow, $maxCol) = findArea($input);
	...
	for ($minRow = 0; $minRow < count($input); $minRow++) {
		for ($maxRow = $minRow; $maxRow < count($input); $maxRow++) {
			for ($minCol = 0; $minCol < count($input[$minRow]); $minCol++) {
				for ($maxCol = 0; $maxCol < count($input[$minRow]); $maxCol++) {
					if (isRect($input, $minRow, $minCol, $maxRow, $maxCol)) {
...
function isRect($input, $minRow, $minCol, $maxRow, $maxCol){
