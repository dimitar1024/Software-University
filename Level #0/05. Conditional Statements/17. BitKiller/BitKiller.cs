...
namespace _17.BitKiller
{
    class BitKiller
    {
        ...
        {
            Console.WriteLine("Problem 17.	** – Bit Killer");
            Console.WriteLine(new string('=', 80));
            Console.WriteLine(@"You are given a sequence of bytes. Consider each byte as sequence of exactly 8 bits.  You are given also a number step. Write a program to remove (kill) all the bits at positions: 1, 1 + step, 1 + 2*step, ... Print the output as a sequence of bytes. In case the last byte have less than 8 bits, add trailing zeroes at its right end. Bits in each byte are counted from the leftmost to the rightmost. Bits are numbered starting from 0.");
            Console.WriteLine(new string('=', 80));
